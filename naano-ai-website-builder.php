...
function naano_activate(): void {
	...
	if ( version_compare( PHP_VERSION, '8.1', '<' ) ) {
		$errors[] = sprintf(
			...
			__( 'Naano AI Website Builder requires PHP 8.1 or higher. You are running PHP %s.', 'naano-ai-website-builder' ),
			PHP_VERSION
		);
	}
	...
	if ( version_compare( $wp_version, '6.0', '<' ) ) {
		$errors[] = sprintf(
			...
			__( 'Naano AI Website Builder requires WordPress 6.0 or higher. You are running WordPress %s.', 'naano-ai-website-builder' ),
			$wp_version
		);
	}
	...
	foreach ( $required_extensions as $ext ) {
		if ( ! extension_loaded( $ext ) ) {
			$errors[] = sprintf(
				...
				__( 'Naano AI Website Builder requires the PHP extension: %s.', 'naano-ai-website-builder' ),
				$ext
			);
		}
	}
...
function naano_deactivate(): void {
	...
	global $wpdb;
	$wpdb->query( "DELETE FROM {$wpdb->options} WHERE option_name LIKE '_transient_naano_%'" );
	$wpdb->query( "DELETE FROM {$wpdb->options} WHERE option_name LIKE '_transient_timeout_naano_%'" );
}
...
function naano_admin_notices(): void {
	...
	if ( ! empty( $errors ) ) {
		...
		echo '<strong>' . esc_html__( 'Naano AI Website Builder could not be activated:', 'naano-ai-website-builder' ) . '</strong><br>';
