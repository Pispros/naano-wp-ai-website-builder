...
jQuery(function($){	// Settings tab switching.
	...
	$('#naano-add-variable-btn').on('click', function(){
		var row = '<tr class="naano-variable-row">' +
			'<td><input type="text" name="naano_vars_keys[]" class="regular-text" placeholder="<?php echo esc_js(
       __("e.g. primary_color", "naano-ai-website-builder"),
   ); ?>"></td>' +
			'<td><input type="text" name="naano_vars_values[]" class="regular-text" placeholder="<?php echo esc_js(
       __("e.g. #3B82F6", "naano-ai-website-builder"),
   ); ?>"></td>' +
			'<td><button type="button" class="button naano-remove-variable"><?php echo esc_js(
       __("Remove", "naano-ai-website-builder"),
   ); ?></button></td>' +
			'</tr>';
	...
	$('#naano-add-language-btn').on('click', function(){
		var row = '<tr class="naano-language-row">' +
			'<td><input type="text" name="naano_lang_codes[]" class="regular-text" placeholder="<?php echo esc_js(
       __("e.g. es", "naano-ai-website-builder"),
   ); ?>" style="max-width:100px;"></td>' +
			'<td><input type="text" name="naano_lang_labels[]" class="regular-text" placeholder="<?php echo esc_js(
       __("e.g. Spanish", "naano-ai-website-builder"),
   ); ?>"></td>' +
			'<td><button type="button" class="button naano-remove-language"><?php echo esc_js(
       __("Remove", "naano-ai-website-builder"),
   ); ?></button></td>' +
			'</tr>';
	...
	$('#naano-save-global-config-btn').on('click', function(){
		...
		$.post(
			<?php echo wp_json_encode(admin_url("admin-ajax.php")); ?>,
			{
				...
				nonce:                      <?php echo wp_json_encode(
        wp_create_nonce("naano_builder_nonce"),
    ); ?>,
			...
			$load.hide();
			$btn.prop('disabled', false);
			$result.show().html('<span class="naano-error">❌ <?php echo esc_js(
       __("Request failed.", "naano-ai-website-builder"),
   ); ?></span>');
		});
	});
	...
	$('#naano-save-api-key-btn').on('click', function(){
		...
		if ( ! apiKey ) {
			$result.show().html('<span class="naano-error">⚠️ <?php echo esc_js(
       __("Please enter an API key.", "naano-ai-website-builder"),
   ); ?></span>');
			$('#naano_api_key').focus();
			return;
		}
       ...
       __(
		...
		$.post(
			...
			function(response){
				...
				if(response.success){
					$result.html(
						'<span class="naano-success">✅ <?php echo esc_js(
          __("Connected!", "naano-ai-website-builder"),
      ); ?> ' +
	...
	$('#naano-test-firecrawl-btn').on('click', function(){
		...
		if ( ! apiKey ) {
			$result.show().html('<span class="naano-error">⚠️ <?php echo esc_js(
       __("Please enter an API key first.", "naano-ai-website-builder"),
   ); ?></span>');
			$('#naano_firecrawl_api_key').focus();
			return;
		}
