...
foreach ($naano_languages as $naano_lentry) {
    if (!empty($naano_lentry["code"])) {
        $naano_lang_map[$naano_lentry["code"]] =
            $naano_lentry["label"] ?? strtoupper($naano_lentry["code"]);
    }
}
...
wp_register_script("naano-admin-pages-list-data", false, [], NAANO_VERSION);
wp_enqueue_script("naano-admin-pages-list-data");
wp_add_inline_script(
    "naano-admin-pages-list-data",
    "window.naanoPagesListData = " .
        wp_json_encode([
...
<div class="wrap naano-builder-wrap">
	...
	<?php // Read-only "deleted=1" success flag after the admin-post trash flow.
 ...
 if (isset($_GET["deleted"])): ?>
	...
	<?php if ($naano_maint_on): ?>
        ...
        "naano-ai-website-builder",
				...
				<?php esc_html_e(
        "Visitors are seeing the maintenance page. Admins (you) still see the real site.",
        "naano-ai-website-builder",
    ); ?>
				<a href="<?php echo esc_url(admin_url("admin.php?page=naano-site-config")); ?>">
					<?php esc_html_e(
         "Manage in Site Configuration →",
         "naano-ai-website-builder",
     ); ?>
				</a>
			</p>
	...
	<?php if (empty($naano_ai_pages)): ?>
		<div class="naano-card" style="padding:30px;text-align:center;">
			...
			<p style="font-size:15px;color:#50575e;">
				<?php esc_html_e(
        "No pages built with Naano AI yet.",
        "naano-ai-website-builder",
    ); ?>
			</p>
		<a href="<?php echo esc_url(
     ...
     // so the slug field appears next to a properly-terminated prefix.
		...
		<div class="naano-card">
			<table class="wp-list-table widefat fixed striped pages">
				<thead>
					<tr>
						<th scope="col" class="column-title column-primary"><?php esc_html_e(
          "Page",
          "naano-ai-website-builder",
      ); ?></th>
         ...
         $naano_section_count = is_array($naano_sections)
             ? count($naano_sections)
             : 0;
         $naano_builder_url = add_query_arg(
             "naano_builder",
             "1",
             get_permalink($page->ID),
         );
         $naano_page_lang = get_post_meta($page->ID, "_naano_lang", true);
         $naano_is_translation = (bool) get_post_meta(
             $page->ID,
             "_naano_translation_of",
             true,
         );
         $naano_root_id = $naano_is_translation
             ? (int) get_post_meta($page->ID, "_naano_translation_of", true)
             : $page->ID;
         $naano_sibling_translations = $naano_is_translation
             ? []
             : get_posts([
                 "post_type" => "page",
                 "post_status" => "any",
         ...
         foreach ($naano_sibling_translations as $naano_st) {
             ...
             if ($naano_tl) {
                 $naano_translated_langs[] = $naano_tl;
             }
         }
         $naano_is_maint = (bool) get_post_meta(
             $page->ID,
             "_naano_maintenance",
             true,
         );
         ...
         ?>
					<tr data-page-id="<?php echo esc_attr($page->ID); ?>">
						<td class="column-title column-primary">
							...
							<a href="<?php echo esc_url($naano_builder_url); ?>" target="_blank">
									<?php echo esc_html(
             $page->post_title ?: __("(no title)", "naano-ai-website-builder"),
         ); ?>
								</a>
							</strong>
						...
						<td>
							<span class="naano-status-badge naano-status-<?php echo esc_attr(
           $page->post_status,
       ); ?>">
								<?php echo esc_html(
            get_post_status_object($page->post_status)->label ??
                $page->post_status,
        ); ?>
							</span>
						</td>
						...
						<td class="naano-perma-cell">
							<div class="naano-perma-wrap" data-page-id="<?php echo esc_attr($page->ID); ?>">
								<span class="naano-perma-prefix"><?php echo esc_html(
            $naano_home,
        ); ?></span><input
									type="text"
					...
					<td>
						<?php if ($naano_page_lang): ?>
							<span class="naano-status-badge naano-status-lang"><?php echo esc_html(
           strtoupper($naano_page_lang),
       ); ?></span>
						<?php endif; ?>
						<?php if ($naano_is_translation): ?>
							<a href="<?php echo esc_url(
           add_query_arg("naano_builder", "1", get_permalink($naano_root_id)),
       ); ?>"
							   class="naano-lang-orig-link" title="<?php esc_attr_e(
              "Go to original page",
              "naano-ai-website-builder",
          ); ?>">
           ...
           $naano_st_lang = get_post_meta(
               $naano_st->ID,
               "_naano_lang",
               true,
           ); ?>
								<a href="<?php echo esc_url(
            add_query_arg("naano_builder", "1", get_permalink($naano_st->ID)),
        ); ?>"
								   class="naano-status-badge naano-status-lang naano-lang-badge--link"
								   title="<?php echo esc_attr(
               $naano_lang_map[$naano_st_lang] ?? strtoupper($naano_st_lang),
           ); ?>">
       ...
       endforeach; ?>
							<?php if (!empty($naano_languages)): ?>
								...
								<div class="naano-translate-form" id="naano-translate-form-<?php echo esc_attr(
            $page->ID,
             ...
             admin_url("admin-post.php"),
               ...
               $naano_llabel =
                   $naano_lentry["label"] ?? strtoupper($naano_lcode);
               $naano_ldone = in_array(
                   $naano_lcode,
                   $naano_translated_langs,
                   true,
               );
               ?>
											<option value="<?php echo esc_attr($naano_lcode); ?>" <?php disabled(
    $naano_ldone,
); ?>>
												<?php echo esc_html(
                $naano_llabel .
                    " (" .
                    strtoupper($naano_lcode) .
                    ")" .
                    ($naano_ldone ? " ✓" : ""),
            ); ?>
											</option>
      ...
      ?>
                ...
                __("Move this page to trash?", "naano-ai-website-builder"),
            ); ?>')"
						>
